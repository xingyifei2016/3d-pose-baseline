...
def load_new_data( names, dpath, take_2d ):
  ...
  if take_2d == 3:
    ...
    print("LOADING GROUNDTRUTH 3D COORDINATES")
...
def create_2d_data( actions, train_names, val_names, train_dir, val_dir, rcams ):
  ...
  print("SAMPLE DATA from 2d GT")
  print("############")
  print(list(train_set.values())[0])
  a = list(train_set.values())[0]
...
def read_mpi ( data_path , do_transform , H36M_mean2d, H36M_mean3d ):
  ...
  if do_transform:
    _, Z, T, b, c = procrustes.compute_similarity_transform(H36M_mean3d.reshape(-1, 3).T, data_mean3d.T, compute_optimal_scale=True)
    test_set3d = np.transpose((b * np.transpose(test_set3d, (0, 2, 1)).dot(T)) + c, (0, 2, 1))
    data_mean3d = np.mean(test_set3d, axis=0)
    data_std3d  =  np.std(test_set3d, axis=0)
...
def read_3d_data( actions, train_names, val_names, train_dir, val_dir, camera_frame, rcams, predict_14=False ):
  ...
  """
  # Load 3d data
  train_set = load_new_data( train_names, train_dir, 2 )
  test_set  = load_new_data( val_names, val_dir, 2 )
...
def postprocess_3d( poses_set ):
  ...
  for k in poses_set.keys():
    ...
    poses = poses_set[k]
    poses = poses[:,3:] - np.tile( poses[:,:3], [1, 16] )
    poses_set[k] = poses
